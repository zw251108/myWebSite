...
<!--                        一种值得尝试的做法是将配置数据存放于非 JavaScript 文件中</p>-->
                ...
                <dd>
                    <p>符合 Web2.0 标准，语义化 html，结构、表现、行为三层分离，兼容性优良，页面性能方便，代码简洁明了有序，尽可能的减少服务器负载，保证最快的解析速度，书写所有人都可以看的懂的代码。简洁易懂是一种美德，为用户着想，为服务器着想</p>
        ...
        <section class="document_section section">
            ...
            <dl class="list">
                ...
                <dd>
                    <p>统一为 html5 声明类型 &lt;!DOCTYPE html&gt;；编码统一为 &lt;meta charset="utf-8" /&gt;（注：要在 head 标签中；在任何其它内容之前，也就是要在 head 中的最前面，包括空格和 &lt;!DOCTYPE html&gt; 文档声明在内，要在前 512 个字节之内）</p>
                ...
                <dd>
                    ...
                    <p>X-UA-Compatible 是用来描述文件在某些浏览器上的渲染程度，不允许设置为 IE=7 来兼容 IE8</p>
                ...
                <dd>永远在 title 元素中给出页面的简单介绍</dd>
                <dt class="icon icon-arrow-r">协议</dt>
                <dd>
                    <p>绝对 URL 都以 HTTP 或 HTTPS 等协议头开始，如果能确定 URL 的协议与当前页面 URL 的协议是一致的，或者说该 URL 在多种协议下均是可用的，则可以考虑删除这个协议头，如果不是这两个声明的 URL 则不能省略</p>

                    <p>省略协议声明，使URL成相对地址，防止内容混淆问题和导致小文件重复下载，如：</p>
                ...
                <dd>
                    <p>非特殊情况下样式文件必须外链至 &lt;head&gt;…&lt;/head&gt; 之间，浏览器在渲染页面时就能尽早的知道每个标签的样式</p>

                    <p>省略掉类型声明 type 属性，除非不用 CSS，因为 HTML5 中默认 type 为 text/css 类型，link 标签的 media 属性若未特殊设置值则的默认值是 all，所以可以省略如：</p>
                ...
                <dd>
                    <p>非特殊情况下 JavaScript 文件必须外链至页面底部 &lt;/body&gt; 之前，对于 JavaScript 来说，因为它在执行过程中会阻塞页面的渲染</p>

                    <p>省略掉类型声明 type 属性，除非不用 JavaScript，因为 HTML5 中默认 type 为 text/javascript 类型；文件须包含库名称及版本号，以及是否为压缩版；引用插件，文件名为库名+插件名称，如：</p>
                ...
                <dd>
                    ...
                    <p>标题根据重要性用 h*(每个根节点与节点元素中只能有一个 h1，备注：hgroup 标签已被取消)</p>

                    <p>段落标记用 p，无序列表 ul，有序列表 ol，定义列表 dl，链接、锚点用 a</p>

                    <p>使用节点元素 section 标签对页面文档结构进行划分，主要对页面上的内容进行层次结构上的划分</p>

                    <p>header 定义页头，footer 定义页脚</p>

                    <p>使用 nav 标签来作为页面导航的元素</p>

                    <p>div、span 标签本身并没有语义，避免使用没有 class 属性的 div、span 标签</p>

                    <p>不允许使用 &lt;b&gt;、&lt;i&gt; 标签来表示加粗和斜体字，使用 &lt;strong&gt; 和 &lt;em&gt; 标签，虽然效果相同，但这是从语义上考虑的，同理，不允许使用 &lt;s&gt;、&lt;strike&gt; 标签来表示文字被删除，可以使用 &lt;del&gt;、&lt;ins&gt; 标签来表示删除与添加</p>

                    <p>保证每个页面可以通过工具（如：Chrome 的 HTML5 Outliner 扩展等）生成合理的文档大纲</p>
                </dd>
                <dt class="icon icon-arrow-r">标签与属性</dt>
                <dd>
                    <p>所有编码均遵循 XHTML 标准，标签、属性、属性值命名必须由小写字母、减号及数字组成（除了文本和 CDATA），且所有标签必须闭合，如：br(&lt;br /&gt;)，hr(&lt;hr /&gt;)等</p>

                    <p>尽可能少使用标签，仅仅使用必要的标签（不仅能让文件保持小巧，下载速度快，浏览器也能够很快完成文档的解析，得到更快的呈现速度），虽然 HTML5 规范中指定了一些可以省略的标签，但不推荐</p>

                    <p>充分利用无兼容性问题的 HTML 自身标签，比如：span、em、strong、optgroup、label 等</p>

                    <p>不允许使用 &lt;blink&gt;、&lt;marquee&gt;、&lt;xmp&gt; 等有兼容性问题的标签</p>

                    <p>页面中所有文字均需要在闭合的标签内</p>

                    <p>属性值必须用双引号包裹</p>

                    <p>不允许使用 font、bgColor、border、align、hspace、vspace 等属性</p>

                    <p>忽略标签的默认属性，如：style、link 标签 的 media 默认值为 all，form 标签的 method 默认值为 get，input 的 type 默认值为 text</p>

                    <p>需要为 HTML 元素添加自定义属性的时候，首先要考虑下有没有默认的已有的合适属性去设置，如果没有，可以使用以 data- 为前缀来添加自定义属性，避免使用 dataX="" 等其他命名方式，如：data-default="123"</p>

                    <p>给重要的元素和截断的元素加上 title</p>

                    <p>原则上不允许使用 style 属性，若页面中的 CSS 代码足够少，则可以写在 style 标签里放在 head 中</p>

                    <p>禁止使用 onclick 等事件属性，若页面中的 JS 代码足够少，则可以写在 script 标签里放在 &lt;/body&gt; 前</p>

                    <p>script 标签的 async 属性对优化非常重要，需要注意的是，如果有多个使用这种方式异步加载的脚本，他们是没有特定的执行顺序的，对于低版本的 IE 浏览器，则尽可能使用 defer 属性</p>

                    <p>在表格定义中，使用 &lt;thead&gt; 来定义表头，使用 &lt;tfoot&gt; 来定义表注，注意 &lt;tfoot&gt; 元素必须出现在 &lt;tbody&gt; 之前，这样浏览器就可以在接受到所有数据之前呈现表注了，使用 &lt;caption&gt; 来定义表格标题，使用 &lt;colgroup&gt; 来定义表格列的分组，使用 &lt;col&gt; 来为表格列定义属性。表格正确结构为：</p>
                    ...
                    <p>img、canvas、video 等标签的 width,height 属性是必填的，可以在浏览器获取其内容前渲染时知道该标签的渲染面积，防止渲染过程中的重绘和回流，这些标签元素对应的 CSS 样式中也需写明 width,height，但是这不意味着通过设置 width,height 来改变缩放图片，除此之外的标签则不允许使用 width,height 属性</p>

                    <p>textarea 标签可以通过 cols 和 rows 属性来规定 textarea 的尺寸（文本输入区内的文本行间，用 "%OD%OA"（回车/换行）进行分隔），可以通过 wrap 属性设置文本输入区内的换行模式</p>
                </dd>
                <dt class="icon icon-arrow-r">class & id 属性</dt>
                <dd>
                    <p>class & id 参见 CSS 书写规范</p>

                    <p>给每一个表格和表单加上一个唯一的、结构标记 id</p>
                </dd>
                <dt class="icon icon-arrow-r">body 上的 id</dt>
                <dd>应用到 body 标签上的 id 叫做 CSS 签名（CSS Signature）用户自定义的样式表可以覆盖页面中的部分甚至所有 CSS 样式，可根据项目需求来使用</dd>
                <dt class="icon icon-arrow-r">accesskey</dt>
                <dd>
                    <p>以下元素支持 accesskey 属性：a、area、button、input、label、legend、textarea，如：</p>
                    ...
                    <p>accesskey 属性规定激活（使元素获得焦点）元素的快捷键，除 Opera 外所有浏览器都支持 accesskey 属性</p>

                    <p>但不同系统与不同浏览器对快捷键的使用方式不同，如：windows 下 ie、chrome 为 alt + 所设的快捷键，firefox 则为 alt + shift + 所设的快捷键</p>

                    <p>随着移动设备的兴起，该属性不再推荐使用，若使用也只建议对导航型的元素来使用</p>
                </dd>
                <dt class="icon icon-arrow-r">标签嵌套</dt>
                <dd>
                    <p>尽可能减少 div 嵌套，div 用来表示结构，一般不要超过 3 层（个人建议：当写你一个 div 标签，该标签的上一层不是 div 标签时，你应该考虑用其它标签替代它了。。。）</p>

                    <p>&lt;a&gt; 里不可以嵌套交互式元素（&lt;a&gt;、&lt;button&gt;、&lt;select&gt; 等）</p>

                    <p>&lt;p&gt; 里面不可以嵌套 &lt;div&gt;、&lt;h1&gt;~&lt;h6&gt;、&lt;p&gt;、&lt;ul&gt;/&lt;ol&gt;/&lt;li&gt;、&lt;dl&gt;/&lt;dt&gt;/&lt;dd&gt;、&lt;form&gt; 等</p>
                    <!--            <p>内联元素中不可嵌套块级元素（在最新的 HTML5 规范中，a 标签似乎可以嵌套块级元素，以便于移动应用）</p>-->
                    <p>删除不必要的标签，如：ul 为块元素，则不需要再使用 div 将其包裹</p>

                    <p>应将 form 标签嵌套在 table 或 ul 外部</p>
                </dd>
                <dt class="icon icon-arrow-r">表单</dt>
                <dd>
                    <p>必须为含有描述性表单元素（input, textarea）添加 label，且 label 的 for 属性必须与表单元素的 id 一一对应</p>

                    <p>若表单元素为 radio,checkbox，则可以用 label 将其嵌套，如：</p>

                    <pre class="brush:html">
                        &lt;label&gt;&lt;input type="radio" name="sex" /&gt;未知&lt;/label&gt;
                    </pre>

					<p>推荐只在表单中使用 input 元素来创建按钮，如果在 HTML 表单中使用 button 元素，不同的浏览器会提交不同的按钮值，IE 将提交 &lt;button&gt; 与 &lt;/button&gt; 之间的文本，而其他浏览器将提交 value 属性的内容。在表单外的按钮使用 button 元素，但始终要为按钮规定 type 属性。不同的浏览器根据 type 属性使用不同的默认值</p>
                </dd>
                <dt class="icon icon-arrow-r">table 标签</dt>
                <dd>
                    <p>尽量避免使用 table 标签，请不要用 width height cellspacing cellpadding 等 table 属性直接定义表现，应尽可能的利用 table 自身私有属性分离结构与表现，在 base.css 文件中初始化表格样式</p>
                ...
                <dd>
                    ...
                    <p>若图片属于“内容”则使用 img 标签，若图片属于样式，则以背景的方式写入 CSS 样式中并使用 sprite 技术（详细参考 CSS 规范相关说明）</p>
                    ...
                    <p>尽量用易懂的词汇，便于团队其他成员理解；另，命名分头尾两部分，用下划线隔开，如：ad_left01.gif 或 btn_submit.gif(建议，可根据具体情况调整)</p>
                    ...
                    <p>图像的 alt 属性会产生冗余，如果使用图像只是为了不能立即用 CSS 来装饰的，就不需要用备选文案了，可以写 alt=""</p>
                </dd>
                <dt class="icon icon-arrow-r">排版</dt>
                <dd>
                    <p>首行缩进处理，不要使用全角空格，使用样式表中的 text-indent:2em; 首字母大写同理使用 text-transform:capitalize;</p>

                    <p>最大程度发挥浏览器的自动排版功能，在一段文字中尽量不要使用 &lt;br/&gt; 来人工干预分段</p>

                    <p>不同语种文字之间应该有一个半角空格，英文字母和数字周围的括号应该使用半角括号，汉字周围的括号应该使用全角括号</p>

                    <p>空白应该尽量使用 text-indent、padding、margin 以及透明的 gif（间隔图片，是 table 布局中常用的一个技巧，通常是一个 1*1 大小的透明 GIF 图片，用在撑大表格等辅助布局中）图片来实现</p>
                ...
                <dd>用一张背景图片替代某元素中的原始文字，以期显示出更美观的字体，这个技巧被称为 "Fahrner 图像替换(Fahrner Image Replacement,FIR)"，该技巧实现非常简单：用标签将元素中文本包围起来，然后通过应用 CSS 样式 display:none; 或 visibility:hidden; 隐藏这个标签，最后将背景图像应用到该元素父级即可。之所以仍保留文字，是为了针对搜索引擎，以及辅助浏览设备（例如，屏幕阅读器即可阅读页面内容，并以声音的形式告知浏览者），但有些屏幕阅读器并不能读取隐藏到的文本，所以更好的办法是不增加额外的标签，而是使用 text-indent:-5000px; 将文本推到屏幕外。</dd>
                <dt class="icon icon-arrow-r">结构与表现分离</dt>
                <dd>
                    <p>禁止使用控制表现的标签，如：&lt;font&gt;&nbsp;&lt;b&gt; 等，字号应该用 CSS 样式表来实现</p>

                    <p>总体上尽量避免在 HTML 中手工编写 CSS/JS（首选的方法是通过工具实现这个过程的自动化）</p>
                </dd>
                <dt class="icon icon-arrow-r">特殊符号</dt>
                <dd>尽可能使用代码替代：比如：&lt;(&amp;lt;)、&gt;(&amp;gt;)、&amp;(&amp;amp;)、空格(&amp;nbsp;)、&raquo;（&amp;raquo;）、版权符号 &copy;(&amp;copy;)等等。</dd>
                ...
                <dd>
                    ...
                    <p>注释格式：&lt;!-- 这是注释 --&gt;，'--' 只能在注释始末位置，不可置入注释文字区域</p>
                ...
                <dd>
                    ...
                    <p>不要把兼容代码和常规代码混合，这样方便代码的维护，如果后期网站不支持这些老旧浏览器，直接删除这些单独的css文件即可，如：</p>
        ...
        <section class="document_section section">
            ...
            <dl class="list">
                ...
                <dd>根据各个模块，同时根据页面相似程度，事先写好共用大体框架文件 base.css，各个前端人员事先规划内部结构、表现与行为；协作开发过程中，每个页面都要引入，此文件包含 reset 及头部底部样式，此文件不可随意修改</dd>
                ...
                <dd>
                    ...
                    <p>id 原则上尽量不用，为 JavaScript 预留钩子的除外，为 JavaScript 预留钩子的命名，请以 js- 或 j- 为前缀，如：js-hide，js-show</p>

                    <p>样式名称由 小写英文、数字、减号和下划线来组合命名，如：main、block-title、btn01；避免使用中文拼音，尽量使用简易的单词组合；总之，命名要语义化，简明化</p>

                    <p>其中减号用来表示状态或类别，如：.module-header, .module-show，下划线用来表示子级元素，如：.module_title, .module_content</p>

                    <p>同时命名中禁止出现描述大小的数字和描述颜色的英文以及所用于的标签（原则上，鼓励每一个 class 可以用在所有类型的标签上）</p>
                </dd>
                <dt class="icon icon-arrow-r">关于 Reset</dt>
                <dd>
                    <p>Reset 的目的为解决浏览器间的差异，统一标签效果，很多的标签本来就没有这个属性或属性本身就是统一的，那么再给设置一次，也有时间的开销，所以不应使用 *{} .class1 *{}，仅应该把你常用到的这些标签进行处理，如：</p>
                ...
                <dd>
                    ...
                    <p>该规则将查找页面上的所有节点，如果节点存在 hidden 属性，且属性值为 true 则匹配成功，这是最耗时的匹配。星号也一样，遍历会消耗很多的时间，如果你的 HTML 代码写的不规范或是某一签标没有必合，这个时间可能还会更长</p>

                    <p>当页面 reflow（重排）时，低效的选择器依然会引发更高的开销</p>
				...
				<dd>
					...
					<p>这样就能在不影响代码私有度的前提下获知 class 作用范围</p>
				</dd>
                ...
                <dd>
                    ...
                    <p>布局定位属性：display list-style position(包括 top,right,bottom,left) float clear visibility overfow</p>

                    <p>自身属性：width height margin padding border background</p>

                    <p>文本属性：color font text-decoration text-align vertical-align white-space</p>
                    ...
                    <p>不建议使用系统颜色（color: WindowText;）</p>

                    <p>CSS 目前支持 17 种颜色名称：black(黑色)、sliver(银色)、gray(灰色)、white(白色)、maroon(栗色)、red(红色)、purple(紫色)、fuchsia(紫红色)、green(绿色)、lime(鲜绿色)、olive(橄榄色)、yellow(黄色)、navy(藏青色)、blue(蓝色)、teal(凫蓝)、aqua(浅蓝绿色)、orange(橙色)，CSS3 将颜色值增加了 130 种</p>

                    <p>从 Gzip 压缩角度，只要整个文档中的书写顺序保持一致即可，可以提高 Gzip 压缩比率</p>
                </dd>
                <dt class="icon icon-arrow-r">CSS 表达式（CSS Expression）</dt>
                <dd>避免使用 CSS 表达式，CSS 表达式只有 IE8 及其以前版本支持，而且他的执行比大多数人想象的要频繁的多，将会严重影响性能，甚至有可能造成无法预料的 JS 错误，不仅页面渲染和改变大小（resize）时会执行，页面滚动（scroll）时也会执行，甚至连鼠标在页面上滑动时都会执行</dd>
                <dt class="icon icon-arrow-r">滤镜</dt>
                <dd>
                    <p>IE 滤镜是很耗性能的，尽量避免使用</p>

                    <p>使用 png 图片时，若 png 中有半透明效果，为 ie6 单独定义背景：</p>
                ...
                <dd>
                    ...
                    <p>在 IE 6 中，设置了 float，并设置的同方向的 margin，会造成 margin 加倍</p>
                ...
                <dd>
                    ...
                    <p>可设置 1px 的内边距来解决此问题</p>
                </dd>
                <dt class="icon icon-arrow-r">em</dt>
                <dd>用 px 作为单位进行绝对定位将能够精确地控制布局，但同时也丧失了布局的灵活性——一旦给定了元素的位置，那么它将始终固定在原地。但若是使用 em 作为 left、top 等属性的单位，元素的位置将变得略加灵活一些——这个位置会根据用户选择不同的字号而相应地改变。若是重叠在一起的若干元素都是用 em 单位，那么任意的变化都将让其余的位置随之调整。虽然定位不是那么精确，但很多时候这种方法却相当好用</dd>
                <dt class="icon icon-arrow-r">居中</dt>
                <dd>
                    <p>使用 text-align 属性可以实现内容的居中，但这种方法并没有将文本属性应用到文本上，而是应用到了作为容器的元素上，所以这属于一种 hack，而且会带来问题，所有子孙元素都会被居中显示</p>

                    <p>更好的做法是使用 margin 来实现居中</p>
                </dd>
                <dt class="icon icon-arrow-r">通用字体族</dt>
                <dd>
                    <p>CSS 内置了一系列的字体族，而所有 CSS 兼容的浏览器都能自动为每个字体族找到至少一个合适的默认字体（当然，用户也可以覆盖默认设置）。CSS 并没有要求我们一定要在字体设定时设定一个字体族作为最后的备选字体，不过这样做却可以防止浏览器在没有匹配的情况下选择其默认的字体，如：</p>
                ...
                <dd>
                    ...
                    <p>字体粗细指文字笔画的宽度，在 CSS 中对应的属性为 font-weight，该属性用来设置文字表现出的粗细。需要注意的是有些字体并没有粗细适中的字样，因而他们并不能提供和 CSS 中等同的粗细划分。而事实上，即使字体有足够多的粗细划分，浏览器也未必能找出最合适的字样。比较现实的做法是仅仅依赖两种设置：normal 和 bold</p>

                    <p>字体宽度指字符的宽度，紧缩字体会显得更为狭窄，而扩张字体则显得宽大，在 CSS 中对应的属性是 font-stretch，不过这个属性尚未被任何浏览器所支持。。。</p>

                    <p>字体姿态，与之对应的 CSS 属性是 font-style。所谓字体姿态实际上是指字体的倾斜程度，有两种：Italic，一种在常规字体基础上做过特殊设计的变体，一般带有手写风格；Oblique，常规字体由浏览器作倾斜处理后的版本。和 Italic 不同的是，Oblique 字体并不出自另外设计的变体，而仅仅是常规字体应用倾斜效果的结果，而多数人则错把 oblique 字体说成 italic 字体</p>

                    <p>如字体设置了 bold、italic 或 oblique 等属性，浏览器一般都会首先尝试选择真实存在的衍生字体。若衍生字体不存在，浏览器就会调整常规字体，让它显示为 bold、italic 或者 oblique 等样式。真实字体和自动调整形成的字体在视觉上有明显的区别，有经验的设计师能很快区分它们</p>
                </dd>
                <dt class="icon icon-arrow-r">字体大小</dt>
                <dd>
                    <p>使用 font-size 来设置字体大小，对所有字体的作用都是相同的</p>

                    <p>不建议使用 em 来设置文字大小，浏览器的默认值为 16 像素，当设置为 0.9em 时，计算结果是 14.4 像素，有的浏览器会取 14 像素，有的浏览器则会取 15 像素。而且，用户若改变了默认设置，那么结果会非常糟糕</p>

                    <p>使用关键字（xx-small,x-small,small,medium,large,x-large,xx-large），每个关键字之间的缩放比例是 1.2，medium 即为默认值 16 像素，也存在上述问题</p>

                    <p>有时候即使设置了字体大小，一些字体还是显得比其它字体更大，这是因为这些字体的 x-height(小写字母去除高出部分和低出部分后的高度) 较大，这种情况下，可以使用 font-size-adjust 属性来解决，但该属性存在兼容性问题</p>
                </dd>
                <dt class="icon icon-arrow-r">文本的样式</dt>
                <dd>
                    <p>font-variant 这个属性的唯一作用是将字母用略小的大写字母表示。原先为大写的字母将仍保持原样，而原先小写的字母将由稍小的大写字母来显示。这个属性有两个合法值，为 normal 和 small-caps，需要指出的是，多数字体没有 small-caps 变体，因而浏览器会将大写字母缩小，因为浏览器具有这种特性，font-variant 也不强制要求字体必须有预设的变体</p>

                    <p>text-transform 提供了更精确的字符大小写控制，可选值有 capitalize、uppercase、lowercase 和 none。capitalize 将每个单词的首字母大写，而 uppercase 将单词所有字母大写，lowercase 则将所有字母小写，none 表示默认的正常状态。</p>

                    <p>text-decoration 可以使下划线获得更多样式选项，但是运用不当将让用户觉得相当不便。这个属性的可选值有 none、underline、overline、linethrough 和 blink。
                        <br/>none 默认值，在没有理由使用其它选项时，none 就是最好的选择。
                        <br/>underline 会为文字添加下划线，仅应该在链接中使用。
                        <br/>overline 会显示出上划线。在数学中，上划线表示平均值，其它领域很少使用。
                        <br/>line-through 会显示为中划线（删除线），仅用于表示已经不再相关、非事实或无效的文字，这些文字可能很快被其它文字替代。理想的用法是用于那些需要被改写的部分，如涉及版本跟踪和版本历史的文字中可以使用。
                        <br/>blink 将会产生时隐时现的效果，不推荐使用，而且并非所有浏览器都能支持
                    </p>
                </dd>
                <dt class="icon icon-arrow-r">间距的样式</dt>
                <dd>
                    <p>line-height 用来控制行间距，可设置绝对数值、相对大小、长度或者百分比。若未加单位则认为是 em，表示基于当前文字大小的相对值</p>

                    <p>letter-spacing 用来控制字符间距，可设置为 normal 或者一个长度值，该长度值将作为默认值的补充，添加到正常的字符间距上。不应该为大段的文字设置字符间距，否则文字将显得很长，因此最好只在标题和小段文字中使用</p>

                    <p>word-spacing 用来控制单词的间距，可设置 normal、长度值或者百分比。需要注意的是 text-align 和 white-space 等属性均会影响到浏览器对 word-spacing 的处理</p>

                    <p>text-align 用来控制文字对齐，可设置为 left、right、center 和 justify</p>
                </dd>
                <dt class="icon icon-arrow-r">中文字体名</dt>
                <dd>
                    <p>样式表中中文字体名，请务必转码成 unicode 码，以避免编码错误时乱码，如：\5b8b\4f53（宋体），\9ED1\4F53（黑体）</p>
                ...
                <dd>
                    ...
                    <p>这种技术和 CSS Sprites 技术都是可以使用构建工具得到的。使用构建工具的好处是不用手工去进行图片的拼合替换，在开发时使用单独的文件就可以。</p>

                    <p>然而坏处是，随着 HTML/CSS 文件的增大增多，必须考虑可能会有一个非常大的图片。如果在 HTTP 请求中没有使用 Gzip 技术压缩 HTML/CSS，那么不推荐使用这种方法，因为减少 HTTP 请求数得到的大文件对于速度来说可能带来相反的结果</p>
                ...
                <dd>
                    ...
                    <p>需要注意的是，若页面中使用了锚点，这些样式也会应用于锚点上，可以添加 id 属性来避免，或是用一种奇怪的语法避免，如：</p>
                ...
                <dd>
                    ...
                    <p>不将 CSS3 属性作为主要功能功能来使用，如动画、变形等效果，也包括 :before,:after</p>
                </dd>
                ...
                <dd>
                    ...
                    <ol>
                        ...
                        <li>background-position:fixed</li>
                        <li>border-radius</li>
                    ...
                    <p>position:absolute; float 也有性能问题（尤其在所包含的子元素很多时），如果使用过多，会让网页变得非常的慢，尽量减少使用</p>
                ...
                <dd>
                    ...
                    <p>写属性值的时候尽量使用缩写，如：font、background</p>
                ...
                <dd>
                    ...
                    <pre class="brush:css">
                        ...
                        @media only screen and (min-device-width:241px) and (max-device-width:360px){
                            /* 针对屏幕宽度于240px与360px宽度之间设备 */
                        }
                        @media only screen and (min-device-width:361px) and (max-device-width:480px){
                            /* 针对屏幕宽度于360px与480px宽度之间设备 */
                        }
                        @media only screen and (min-device-width:481px) and (max-device-width:640px){
                            /* 针对屏幕宽度于480px与640px宽度之间设备 */
                        }
                        @media only screen and (min-device-width:641px) and (max-device-width:800px){
                            /* 针对屏幕宽度于640px与800px宽度之间设备 包括：Nokia900（横向） */
                        }
                        @media only screen and (min-device-width:801px) and (max-device-width:1024px){
                            /* 针对屏幕宽度于800px与1024px宽度之间设备 包括：iPad（横向） */
                        }
                        @media only screen and (min-device-width:1025px){
                            /* 针对屏幕宽度超过1024px设备 包括：普通显示器 */
                        }
                    </pre>

                    <p>在此列出的只是一些可能情况（也许还可以再补充），实际开发时可将几种情况合并，备注：在 HTML 页面中需要设置 meta 标签，详细见 HTML 规范</p>
                </dd>
                <dt class="icon icon-arrow-r">CSS3 动画</dt>
                <dd>
                    <p>使用 CSS 动画比起 JavaScript 驱动的动画效率更高，CSS 动画同时需要更少的代码，很多的 CSS 动画是用 GPU 处理的，因此动画本身很流畅，可以使用下面代码强制使你的硬件加速：</p>
                ...
                <dd>
                    ...
                    <p>可以在开发时将所有 CSS 代码嵌入到 HTML 页面中的 &lt;style&gt; 标签中，即可避免许多因浏览器缓存导致的不正常现象，但在发布前，一定要将这些样式移至外部文件，然后用 &lt;link&gt; 标签引入</p>
                </dd>
            </dl>
        </section>
        <section class="document_section section">
            ...
            <dl class="list">
                ...
                <dd>
                    ...
                    <p>代码结构明了，加适量注释，提高函数重用率，尽量避免使用存在消耗资源及兼容性的方法或属性，如：eval() 或 innerText 等</p>

                    <p>注重与 html 分离，减小 reflow，注重性能</p>

                    <p>原则上所有功能均根据 XXX 项目需求原生开发，以避免网上 Download 下来的代码造成的代码污染（冗余代码或与现有代码冲突等情况）</p>
                </dd>
                ...
                <dd>
                    <p>网站的开发经常回调用第三方的内容，例如：天气、分享等，问题在于，不管是用客户端还是服务器端的连接，都无法保证这些代码是正常有效工作的。这些服务有可能临时 Down 掉或是被用户或者其公司的防火墙阻止</p>
                ...
                <dd>
                    <p>这个（坏）方法已经被开发者抛弃了很多年，但是在某些情况下仍然是需要的，例如在一些 JavaScript 文件的同步回退中，如：</p>
                ...
                <dd>
                    <p>truthy 和 falsy 的概念：它们是一种 true/false 字面量。在 JavaScript 中，所有的非 Boolean 型值都会内置一个 boolean 标志，当这个值被要求有 boolean 行为的时候，这个内置布尔值就会出现，例如当你要跟 Boolean 型值比对的时候</p>

                    <p>当 JavaScript 两个不同类型的值要求做比较的时候，它首先会将其弱化成相同的类型。false, undefined, null, 0, "", NaN 都弱化成 false</p>

                    <p>无关键字的数组等同于 false，如：</p>
                    ...
                    <p>为避免引起混乱的隐含类型转换，在比较值和表达式类型的时候始终使用 === 和 !== 操作符</p>
                </dd>
                <dt class="icon icon-arrow-r">对象比较</dt>
                <dd>
                    <p>当在进行比较时，如果其中一个值是对象而另一个不是，则会首先调用对象的 valueOf() 方法，得到原始类型值再进行比较，如果没有定义 valueOf()，则调用 toString()，如：</p>
                ...
                <dd>
                    ...
                    <p>原生 JavaScript 变量要求是纯英文字母，首字母须小写，如：var value = 1;</p>

                    <p>jQuery 变量要求首字符为 '$'，其它与原生 JavaScript 规则相同，如：var $container = $('div.container');</p>

                    <p>建议对象私有属性和私有方法以下划线开头</p>

                    <p>使用对象及数组字面量而不是更冗长的声明</p>

                    <p>变量名应该尽可能短，尽量在变量名字体现出值得数据类型，如：count、length 和 size 表明数据类型是数字，而命名 name、title 和 message 表明数据类型是字符串，单个字符命名的变量如 i、j 等通常用在循环中</p>
                </dd>
                ...
                <dd>
                    ...
                    <p>对于函数和方法命名来说，第一个单词应该是动词，如以 can、has、is 开头的函数或方法应返回一个布尔值，以 get 开头返回一个非布尔值，以 set 开头的函数或方法用来保存一个值，如：</p>
                ...
                <dd>用单引号定义字符串，JavaScript 中可以用单引号或者双引号定义字符串，但是因为习惯于在 HTML 中元素的属性值的定义使用双引号，而 JavaScript 中又经常包含 HTML 代码，所以字符串定义使用单引号也是方便于在字符串内部包含含有双引号的 HTML 代码</dd>
                <dt class="icon icon-arrow-r">字符串拼接</dt>
                <dd>
                    <p>对于 IE6/7 使用 Array.join() 连接大量字符串的效率要高于使用 + 元素运算符。但目前主流的浏览器，包括 IE8 以后的版本，都对 + 元素运算符连接字符串做了特别优化，性能已经显著高于 Array.join()。目前大多数情况下，建议连接字符串首选使用 + 元素运算符</p>

                    <p>使用 Array.join() 方法连接字符串，如：</p>
                ...
                <dd>
                    ...
                    <p>二进制的浮点数不能正确处理十进制的小数，因此 0.1 + 0.2 不等于 0.3。这是 JavaScript 中最常见的 bug，但它是遵循二进制浮点数算术标准（IEEE 754）而有意导致的结果</p>

                    <p>NaN 是一个特殊的数量值，它表示不是一个数字，但 typeof NaN 的结果是 'number'，而且 NaN 不等同于它自己，可以用 isNaN() 来判断数字与 NaN</p>

                    <p>判断一个值是否可用做数字的最佳方法是 isFinite 函数，它会筛除掉 NaN 和 Infinity，但它会试图把参数转换为一个数值，所以可以这样定义 isNumber 函数：</p>
                ...
                <dd>
                    ...
                    <p>instanceof 运算符也可以检测自定义的类型，如：</p>
                    ...
                    <p>在 JavaScript 中检测自定义类型时，最好的方法就是使用 instanceof 运算符。但是，有一个严重的限制，假设一个浏览器帧（frameA）里的一个对象被传入另一个帧（frameB）中。两个帧里都定义了构造函数 Person。如果来自帧 A 的对象是帧 A 的 Person 的实例，则有如下结果：</p>
                    ...
                    <p>因为每个帧（frame）都拥有 Person 的一份拷贝，它被认为是该帧（frame）中的 Person 的拷贝的实例，尽管两个定义可能完全一样，函数和数组也有这个问题</p>
                </dd>
                <dt class="icon icon-arrow-r">检测函数</dt>
                <dd>
                    <p>检测函数最好的方法是使用 typeof，但有一个限制，在 IE8 和更早版本 IE 浏览器中，使用 typeof 来检测 DOM 节点（比如 document.getElementById()）中的函数都会返回 "object"，这种怪异现象是因为浏览器对 DOM 的实现有差异，所以要通过 in 运算符来检测 DOM 的方法，如：</p>
                ...
                <dd>
                    <p>ECMAScript5 将 Array.isArray() 正式引入 JavaScript，但为为了兼容低版本浏览器，使用：</p>
                ...
                <dd>
                    <p>判断属性是否存在的最好方法是使用 in 运算符，in 运算符仅仅会判断属性是否存在，而不会去读属性的值</p>
                    ...
                    <p>如果实例对象的属性存在或者继承自对象的原型，in 运算符都会返回 true</p>
                </dd>
                <dt class="icon icon-arrow-r">检测属性</dt>
                <dd>如果只想检测实例对象的某个属性是否存在，使用 hasOwnProperty() 方法。所有继承自 Object 的 JavaScript 对象都有这个方法，需要注意的是 IE8 以及更早版本的 IE 中，DOM 对象并非继承自 Object，因此不包含此方法</dd>
                <dt class="icon icon-arrow-r">null</dt>
                <dd>
                    <p>在下列场景中应当使用 null</p>

                    <ul>
                        <li>用来初始化一个变量，这个变量可能赋值为一个对象，这样即使未赋值，typeof 运算符也会返回 "object"</li>
                    ...
                    <ul>
                        <li>不要用 null 来检测是否传入了某个参数</li>
                        <li>不要用 null 来检测一个未初始化的变量</li>
                    </ul>
                </dd>
                <dt class="icon icon-arrow-r">undefined</dt>
                <dd>避免在代码中使用 undefined，可以有效地确保只在一种情况下 typeof 才会返回 "undefined"：当变量未声明时</dd>
                ...
                <dd>
                    <p>代码处理分两个阶段，第一阶段是变量，函数声明，以及正常格式的参数创建，这是一个解析和进入上下文的阶段。第二个阶段是代码执行，函数表达式和不合格的标识符（未声明的变量）被创建，在第一阶段，所有使用 var 声明的变量都会被创建，所以当该变量在 var 语句前即被使用时，该变量的值会是 undefined，这就是变量提升。</p>

                    <p>变量提升意味着：在函数内部任意地方定义变量和在函数顶部定义变量是完全一样的。为了防止变量提升，应先声明后使用，并遵循单一 var 模式</p>
                </dd>
                <dt class="icon icon-arrow-r">引用</dt>
                <dd>若两次以上引用同一对象的属性，应该定义到局部变量再引用</dd>
                <dt class="icon icon-arrow-r">全局变量</dt>
                <dd>
                    <p>尽量不要定义全局变量，全局变量的问题在于，你的 JavaScript 应用程序和 Web 页面上的所有代码都共享了这些全局变量，他们住在同一个全局命名空间，所以当程序的两个不同部分定义同名但不同作用的全局变量的时候，命名冲突在所难免</p>

                    <p>为防止意料之外的全局变量，所有变量都要使用 var 来声明，以防隐式全局变量的出现，也不要使用任务链进行部分 var 声明，如：</p>
                ...
                <dd>
                    <p>条件判断不要省略大括号，将开放的大括号放置在前面语句的同一行，当条件判断不超过 3 个的时候使用 if/else，若超过 3 个使用 switch，条件越多时 switch 效率越高，同时条件判断按概率从最大到最小的顺序排列</p>

                    <p>使用 switch 时遵循如下风格:
                        <br/>每个 case 和 switch 对齐（花括号缩进规则除外）
                        <br/>每个 case 中代码缩进
                        <br/>每个 case 以 break 清除结束，避免贯穿（switch Fall Through，故意忽略 break，使执行完一个 case 后续连续执行下一个 case）
                        <br/>以 default 结束 switch，确保总有健全的结果，即使无情况匹配
                    </p>

					<p>如下使用 switch/case 判断数值范围的时候是合理的，但一般对于数值范围的判断，用 if/else 会比较合适。 switch/case 更适合对确定数值的判断</p>
                ...
                <dd>
                    ...
                    <p>JavaScript 原生循环方法 for 和 while，要比 jQuery.each() 方法快</p>

                    <p>不要循环引用</p>

                    <p>对循环进行优化，如访问数组时，存储数组长度，减少对数组的 length 访问（每次访问是都会对数组进行统计），尤其在访问 var items = document.getElementsByTagName('a') 等类似方法生成的 HTML 节点数组（NodeList）时，存储数组长度尤为关键。这些集合通常被认为是“活的”，也就是说，当他们所对应的元素发生变化时，他们会被自动更新</p>

                    <p>同时如果可能，使用倒序访问数组（可以减少一次判断），当使用倒序访问时，也可以使用 while 循环，如：</p>
                    ...
                    <p>建议使用 i = i + 1 或 i += 1 的表达式来替换 i++</p>

                    <p>for-in 循环是用来遍历对象的，不允许用来遍历数组，同时尽量不使用 for-in 循环（for 循环的效率比 for-in 快 7 倍），若使用则在遍历对象时过滤原型继承的属性，如：</p>
                ...
                <dd>
                    <p>在 JavaScript 中抛出错误要比在任何其它语言中做同样的事情更加有价值，这归咎于 Web 端调试的复杂性。使用 throw 操作符，将提供的一个对象作为错误抛出，任何类型的对象都可以作为错误抛出，然而 Error 对象是最常用的，如果没有通过 try-catch 语句来捕获的，浏览器会直接显示该消息，如：</p>
                    ...
                    <p>用 try-catch 语句来捕获异常，当在 try 块中发生一个错误时，程序立刻停止执行，然后跳到 catch 块，并传入一个错误对象，还可以增加一个 finally 块，finally 块中的代码不管是否有错误发生，最后都会被执行，如：</p>
                    ...
                    <p>在某些情况下，finally 块工作起来有点复杂，如 try 块中包含了一个 return 语句，实际上它必须等到 finally 块中的代码执行后才能返回</p>

                    <p>ECMA-262 规范指出了 7 种错误类型：</p>

                    <ul>
                        <li>Error 所有错误的基本类型，实际上引擎从来不会抛出该类型的错误</li>
                        <li>EvalError 通过 eval() 函数执行代码发生错误时抛出</li>
                        <li>RangeError 一个数字超出它的边界时抛出，该错误在正常的代码执行中非常罕见</li>
                        <li>ReferenceError 期望的对象不存在时抛出</li>
                        <li>SyntaxError 给 eval() 函数传递的代码中有语法错误时抛出</li>
                        <li>TypeError 变量不是期望的类型时抛出</li>
                        <li>URIError 给 encodeURI()、encodeURIComponent()、decodeURI() 或 decodeURIComponent()
                            等函数传递格式非法的 URI字符串时抛出</li>
                    </ul>
                    ...
                    <p>抛出其它类型的的对象，可以区分于浏览器的错误，但也会错过 Error 对象附加的一些额外信息。解决方案是，创建自己的错误类型，让它继承自 Error，如：</p>
					...
					<p>避免在循环内部使用 try-catch</p>
                </dd>
                <dt class="icon icon-arrow-r">数组</dt>
                <dd>
                    <p>JavaScript 的数组只是一种拥有一些类数组特性的对象，它把数组的下标转变成字符串，用其作为属性。它明显比一个真正的数组慢，但它使用起来更方便</p>

                    <p>可以直接设置 length 的值，把 length 设小将导致所有下标大于等于新 length 的属性被删除</p>

                    <p>数组的 splice() 方法可以删除一些元素并将它们替换为其它的元素，因为被删除属性后面的每个属性必须被删除，并且以一个新的键值重新插入，这对于大型数组来说可能会效率不高</p>

                    <p>数组中的 shift() 方法通常比 pop() 方法慢很多</p>
                ...
                <dd>
                    <p>如果正则表达式带有 g 标识，查找不是从这个字符串的起始位置开始，而是从正则表达式的 lastIndex 位置开始，如果匹配成功，那么 lastIndex 将设置为该匹配后的第一个字符的位置，不成功的匹配会重置 lastIndex 为 0</p>

                    <p>不要对 test() 方法使用 g 标识</p>

                    <p>\1 指向分组 1 所捕获的文本的一个引用，如：</p>
                    ...
                    <p>当 String.split() 方法的参数是一个正则表达式的时候，IE8 及其以前版本浏览器会在输出的数组结果中排除空字符串</p>
                </dd>
                <dt class="icon icon-arrow-r">with</dt>
                <dd>with 语句可以更改包含的上下文解析变量的方式，通过 with 可以用局部变量和函数的形式来访问特定对象的属性和方法，这样就可以将对象前缀省略掉。但是 with 带来的坏处很多，阅读代码时，很难分辨变量是局部变量还是对象的属性，JavaScript 引擎和压缩工具无法对代码进行优化，而且在最新的严格模式中，with 语句是被禁止的，所以应避免使用 with 语句</dd>
                <dt class="icon icon-arrow-r">JSON</dt>
                <dd>
                    <p>使用 JSON 格式进行数据交换，JSON 是 JavaScript 源生格式，处理 JSON 数据不需要特殊 API 或工具包</p>

                    <p>推荐使用标准的 JSON 格式，要求必须只能使用双引号作为键或值得边界符号，不能使用单引号，而且键必须使用边界符，如：</p>
                ...
                <dd>使用 parseInt() 可以从字符串中获取数值，该方法接受另一个基数参数，这经常省略，但不应该。当字符串以 "0" 开头的时候就有可能出现问题，开头为 "0" 的字符串被当做 8 进制处理，这已在 ECMAScript 5 中改变了，但为避免矛盾和意外的结果，应该指定基数参数</dd>
                <dt class="icon icon-arrow-r">Date</dt>
                <dd>Date 可以接受数值型的参数，可以为 13 位数字的时间戳，也可以为年、月、日、时、分、秒的数值，但是也可以接受字符串参数，但是有严格的格式要求，以 yyyy/mm/dd hh:ii:ss 形式，其中时分秒部分可以省略，也可以只省略最后秒的部分</dd>
                <dt class="icon icon-arrow-r">setInterval/setTimeout</dt>
                <dd>
                    <p>不要给 setInterval 或 setTimeout 传递字符串参数</p>
                ...
                <dd>
                    ...
                    <ul>
                        <li>原生对象（如：Object、Array）</li>
                        <li>DOM 对象（如：document）</li>
                        <li>浏览器对象模型（BOM）对象（如：window）</li>
                        <li>类库对象</li>
                    </ul>
                ...
                <dd>
                    ...
                    <p>在 JavaScript 中，继承仍有一些很大的限制。不能从 DOM 或 BOM 对象继承，由于数组索引和 length 属性之间错综复杂的关系，继承自 Array 是不能正常工作的</p>
                </dd>
                <dt class="icon icon-arrow-r">基于类型的继承</dt>
                <dd>
                    <p>基于类型的继承是通过构造函数实现的，而非对象。基于类型的继承一般需要两步：首先，原型继承；然后，构造器继承。构造器继承是调用超类的构造函数时传入新建的对象作为其 this 的值，如：</p>
                ...
                <dd>
                    <p>基于对象的继承，也经常叫作原型继承，一个对象继承另外一个对象是不需要调用构造函数的，ECMAScript5 的 Object.create() 方法是实现这种继承的最简单的方式，如：</p>
                ...
                <dd>
                    <p>随着 ECMAScript5 和 HTML5 的特性开始在各种浏览器中的实现，JavaScript polyfills 变得流行起来了。一个 polyfill 是指一种功能（这些功能在新版本的浏览器中已经有完备定义并原生实现了）的模拟，以便在老版本的浏览器中如同新版本一样使用。polyfills 的关键是它们模拟的原生功能要以完全兼容的方式来实现，因此在有些浏览器中存在了这些功能，所以有必要检测不同情况下它们的处理是否符合标准的方式。为了达到目的，polyfills 经常会给非自己拥有的对象新增一些方法。polyfills 的优点是，当只支持浏览器的原生功能时，它们非常容易删除；缺点是，它们可能没有精确地实现它们（原生浏览器环境）所缺失的功能，从而带来的麻烦比缺失的功能要多得多</p>

                    <p>从最佳的可维护性角度而言，避免使用 polyfills，如果选择使用某个 polyfill，要做好严格审查，要保证它的功能和原生版本尽可能的近似</p>
                </dd>
                ...
                <dd>
                    <p>因为各浏览器之间的行为的差异，我们经常需要获得浏览器的类型，通常通过 JavaScript 的 navigator.userAgent 来判断，但随着每一个新浏览器出现，用于用户代理检测的代码都需要更新，最安全的方法是只检测旧的浏览器，如 IE8 及以前版本</p>
                ...
                <dd>
                    <p>当有任何属性或元素发生改变时，都会引起 DOM 元素的重绘或回流，毫无疑问，应当避免过多的重绘和回流</p>

                    <p>当一个元素的布局不变，外观发生改变时，就会引起重绘，例如改变 background-color</p>

                    <p>当改变一个页面的布局时就会发生回流，例如改变一个元素的宽，回流的代价是最高的</p>

                    <p>例如：当你设置 style.width 时，浏览器需要重新计算布局。通常浏览器暂时是不需要知道改变了元素的样式的，直到它需要更新屏幕时，正因为如此，改变多个元素的样式只会产生一次回流。</p>

                    <p>优化方法：</p>

                    <ul>
                        <li>将多次改变样式属性的操作合并成一次操作，通过设置 DOM 节点的 class 的方式减少样式修改次数</li>
                        <li>减少 DOM 元素的几何属性变化</li>
                        <li>减少 DOM 树的结构变化</li>
                        <li>减少获取某些属性，包括 offsetTop、offsetLeft、offsetWidth、offsetHeight、scrollTop、scrollLeft、scrollWidth、scrollHeight、clientTop、clientLeft、clientWidth、clientHeight、getComputedStyle()(currentStyle in IE)</li>
                        <li>将需要多次重排的元素，position 属性设为 absolute 或 fixed、relative，这样此元素就脱离了文档流，它的变化不会影响到其他元素。例如有动画效果的元素 position 设为 fixed 或 absolute</li>
                        <li>在内存中多次操作节点，完成后再添加到文档中去。例如要异步获取表格数据，渲染到页面。可以先取得数据后在内存中构建整个表格的 html 片段，再一次性添加到文档中去，而不是循环添加每一行</li>
                        <li>由于 display 属性为 none 的元素不在渲染树中，对隐藏的元素操作不会引发其他元素的重排。如果要对一个元素进行复杂的操作时，可以先隐藏它，操作完成后再显示。这样只在隐藏和显示时触发 2 次重排</li>
                        <li>在需要经常取那些引起浏览器重排的属性值时，要缓存到局部变量</li>
                        <li>最简化 DOM 结构，将 HTML 标签数量降到最低，当 JS 引发的浏览器重排与重绘事件不可避免时，只能减少所引发的操作量</li>
                        <li>合理规划 JS 的事件，使其影响到的 HTML 标签最少 DOM 树</li>
                    </ul>
                </dd>
                <dt class="icon icon-arrow-r">高频执行事件/方法的防抖</dt>
                <dd>
                    <p>通常会在用户交互参与的地方添加事件，而往往这种事件会被频繁触发，比如 scroll、mouseover 事件，它们触发时，执行的非常迅速，并且触发很多次，如果回调过重，浏览器可能会 Down 掉，引入防抖来限制一个方法在一定时间内的执行次数，如：</p>
                ...
                <dd>
                    <p>因为各浏览器之间的行为的差异，我们经常会在函数中包含了大量的 if 语句，以检查浏览器特性，解决不同浏览器的兼容问题，解决的方案就是称之为惰性载入的技巧，如：</p>
                    ...
                    <p>在这个惰性载入的 doSomething() 中，if 语句的每个分支都会为 doSomething 变量赋值，有效覆盖了原函数。最后一步便是调用了新赋函数。下一次调用 doSomething() 的时候，便会直接调用新赋值的函数，这样就不用再执行 if 语句了。</p>
                ...
                <dd>
                    <p>级联，当自定义一个对象时，若其的方法没有特定的返回值时，使其返回 this，目的是分割函数，以创建更加简短、具有特定功能的函数，提高了代码的可维护性，同时也可以节省一些输入的字符，但缺点在于代码更加难以调试，如：</p>
                ...
                <dd>
                    <p>强烈推荐使用文档生成工具来为你的 JavaScript 生成文档，当使用文档注释时，应确保对所有的方法、所有的构造函数、所有包含文档化方法的对象添加注释。注释的详细格式和用法最终还是由所选择的文档生成工具所决定的，如：</p>
                ...
                <dd>为了编写可维护的 JavaScript，测试驱动是必不可少的。因为每个测试都起到质量反馈的作用，给后期维护和修改创建了一个安全保护网并提供了一份可执行的文档。通过测试，我们可以保证所有的功能被覆盖，也避免了重写代码后再进行测试的高昂代价。</dd>
            </dl>
        </section>
        <section class="document_section section">
            ...
            <dl class="list">
                ...
                <dd>GIF 使用一种叫做 LZW 的无损压缩格式，非常适合带有大块相同颜色区域的图像。单色插图、文本、色块等文件非常适合存为此格式。GIF 使用颜色索引来实现无损压缩，该格式的色深只允许同一张图片中最多出现 256 种颜色，对于超过范围的若干种颜色只能忍痛割爱。但对于少于 256 种颜色的图像，其颜色索引也会相应地变小，这也就意味着图片文件的整体大小也将随之降低。当然，对于超过 256 种颜色的图像，仍可以将其存为 GIF 格式，不过某些颜色信息将丢失——最终变成有损压缩。此外，GIF 还支持基于帧的简单动画以及隔行加载技术，但这两者都将增加文件大小</dd>
                <dt class="icon icon-arrow-r">JPEG</dt>
                <dd>JPEG 的 24 位色深让其能够很好的显示照片等色彩、细节丰富的图像。JPEG 支持有损压缩且可以设置压缩等级，压缩比很大的 JPEG 图像将会高度失真，并引发一种叫做非自然痕迹的小像素缺陷问题。虽然图像中的丰富细节将会淡化非自然痕迹的影响，但在使用 JPEG 是仍要把握好图像质量和文件大小之间的平衡关系，JPEG 不支持任何形式的透明度，最新的“渐进 JPEG(Progressive JPEG)”版本能够和 GIF 一样实现隔行加载</dd>
                <dt class="icon icon-arrow-r">PNG</dt>
                <dd>PNG 格式在有些方面与 GIF 类似，但它提供了两种色深模式：24 位和 8 位。24 位色深的 PNG 通常都很大，但却能保证完全无损，原始图片的所有细节都将被精确地保留。8 位色深的 PNG 文件使用了类似 GIF 的颜色索引，但很多时候，同样设置下用 PNG 压缩的图像要比 GIF 更小。PNG 还支持两种模式的透明度设置：1 位（bit）或 8 位。前者与 GIF 的行为一样，而后者则能够实现 256 级的透明度，但不幸的是，IE6 不能正确呈现出 PNG 的透明度设置</dd>
                <dt class="icon icon-arrow-r">无损压缩</dt>
                <dd>图片文件中包含许多对于 Web 来说没用的东西，例如：JPEG 图片中可能包含 Exif 元数据（数据，相机型号，坐标等），PNG 图片会包含有关颜色，元数据的信息，有时甚至还包含一个缩略图，应利用自动化工具来删除这些信息，这个被称作无损压缩
                </dd>
                <dt class="icon icon-arrow-r">有损压缩</dt>
                <dd>以图片质量为代价进行压缩，称为有损压缩。例如：处理 PNG 图片时，常见的是减少颜色数量，或者将 PNG-24 格式转换为 PNG-8 格式；处理 JPEG 图片时，可以选择导出质量（0 到 100），选择可接受范围内的最低值</dd>
                <dt class="icon icon-arrow-r">透明</dt>
                <dd>GIF 支持 1 位的透明度，即某个像素要么完全透明，要么完全不透明，没有任何中间值，所以使用时必须格外小心。所有其他半透明的像素都要恰当地与背景颜色融合在一起，即所谓的“蒙版颜色”。蒙版必须与该 GIF 周围的背景颜色协调一致，否则将呈现出明显的不协调边缘</dd>
                <dt class="icon icon-arrow-r">优化</dt>
                <dd>
                    <p>优化图片大小的诀窍在于适当牺牲一定的图像质量</p>

                    <p>优化 GIF 的步骤通常是从最低的颜色级别开始，逐步提高颜色数目。直至可以接受。若从 8 种颜色开始，那么接下来就依次查看 16、32、64、128、256 种颜色的样子，直到你觉得满意。某些图像确实需要或超过 256 种颜色，则考虑使用 JPEG 代替 GIF。还有一些设定能够影响到 GIF 格式文件的大小。例如抖动技术（dither）即可在一块相同颜色区域中添加另一种颜色的像素，这个技术可以用于模拟出一些本不在文件颜色索引中的颜色。但抖动技术也将影响文件的压缩比率，若是只考虑降低文件大小，那么某些时候增加颜色种类可能要比使用抖动技术更为有效</p>

                    <p>优化 JPEG 很简单，在保存为 JPEG 格式时，选择品质等级即可，最好的方式是从较低品质（即较高压缩比）的图像开始，逐步提高品质，直到对图像质量满意为止，同时从性能考虑，选择可接受范围内的最低值，为了提升用户体验，还应将你的 JPEG 文件转换为渐进式的。现在大多数浏览器都支持渐进式 JPEG 文件，并且这个格式文件创建简单，没有明显的性能损失问题，页面中这种格式的图片能够更快的展现</p>

                    <p>使用 png 图片时，要求图片格式为 png-8 格式，若 png-8 实在影响图片质量，再使用其它格式，优化 8 位 PNG 图像的步骤与优化 GIF 完全一致，24 位 PNG 图像的优化更为简单——无需任何关注。因为 PNG 的唯一压缩方式固化在文件格式中，根本无法更改</p>
                </dd>
        ...
        <section class="document_section section">
            ...
            <dl class="list">
                ...
                <dd>
                    ...
                    <p>实时性不太好是 CDN 的致命缺陷，解决方法是在网络内容发生变化时将新的网络内容从服务器端直接传送到缓存器，或者当对网络内容的访问增加时将数据源服务器的网络内容尽可能实时地复制到缓存服务器</p>
                </dd>
                <dt class="icon icon-arrow-r">GZIP</dt>
                <dd>服务器端启用 GZIP 压缩</dd>
				<dt class="icon icon-arrow-r">支持浏览器缓存</dt>
				<dd>如果浏览器能缓存一个文件，那就无需反复下载该文件了，在 HTTP 头中设置合适的 Expires Header、上一次修改时间或采用的 ETags</dd>
        ...
        <section class="document_section section">
            ...
            <dl class="list">
                ...
                <dd>
                    <p>以下为按速度排序（以 $ 开头的为 jQuery 对象）</p>

                    <ol>
                        <li>$parent.find('.child')</li>
                        <li>$('.child', $parent)</li>
                        <li>$('.child', $('#parent') )</li>
                        <li>$parent.children('.child')</li>
                        <li>$('#parent > .child')</li>
                        <li>$('#parent .child')</li>
                    </ol>
                </dd>
                ...
                <dd>
                    <p>改动 DOM 结构开销很大，不要频繁使用 .append()、.insertBefore() 和 .insertAfter() 等方法</p>

                    <p>如果要插入多个元素，就把它们合并，一次性插入</p>

                    <p>如果对一个 DOM 元素进行大量处理，应先使用 .detach() 方法，处理完毕后，再重新插回文档</p>

                    <p>插入 HTML 代码的时候，源生的 innerHTML 比 jQuery 的 html() 方法更快</p>

                    <p>jQuery 的文档操作接口根据编程的不同场景来划分使用它们</p>

                    <p>如果需要插入硬编码的元素或者跟在静态实例后面的元素，使用 .append() 方法</p>

                    <p>如果需要处理包含多个元素或者动态数据的对象，使用 .appendTo() 方法</p>
                </dd>
                <dt class="icon icon-arrow-r">样式变更</dt>
                <dd>
                    <p>要给少数的元素加样式，通常的方法是使用 jQuery 的 css() 函数，然而更15个以上的较多的元素添加样式时，直接给 DOM 添加 style 标签更有效些。这个方法可以避免在代码中使用硬编码(hard code)。如：</p>
                ...
                <dd>
                    ...
                    <p>父级代理多个子级事件时，不要使用 CSS 中已定义的 class 样式名称，而要自定义，防止结构变化对功能的影响</p>
                    ...
                    <p>不要对非交互型的标签绑定键盘事件（keydown，keyup，keypress），尤其不要对 document 绑定，仅仅建议对 input:text，textarea 等用户可输入类型的控件进行键盘事件绑定</p>
                </dd>
                <dt class="icon icon-arrow-r">结构、样式与功能分离</dt>
                <dd>
                    <p>为防止 CSS 代码样式变更，对一个 class 绑定事件时，不要使用 CSS 样式文件中已定义的样式 class，而使用自定义的 CSS class 名，最好遵从某些规律，推荐以 j- 或 js- 为前缀</p>

                    <p>同时为防止 HTML 代码层级结构变化，尽量不使用固定结构的方法，如：.next(),.prev(),.parent() 等。而使用 .parents() 找到模块级别的父级再使用 .find() 来执行</p>
                </dd>
                ...
                <dd>采用 jQuery 的内部函数 data() 来存储状态</dd>
                <dt class="icon icon-arrow-r">jQuery utility</dt>
                <dd>使用 jQuery 的 utility 函数，如 $.isFunction(),$.isArray() 等</dd>
        ...
        <section class="document_section section">
            ...
            <dl class="list">
                ...
                <dd>
                    ...
                    <p>触发条件：给元素设置了 float 并且同时和 float 同一方向设置了 margin 值</p>

                    <p>解决方案：浮动元素中增加一个 display:inline; 属性</p>
                </dd>
                <dt class="icon icon-arrow-r">不识别 min-height、min-width 属性</dt>
                <dd>
                    <p>浏览器版本：IE 6</p>

                    <p>触发条件：使用了 min-height、min-width 属性</p>

                    <p>解决方案：使用 height:auto !important; 解决</p>
                </dd>
...
<!--                </dd>-->
                ...
                <dd>
                    ...
                    <p>触发条件：设置元素的字体大小小于 12px</p>

                    <p>解决方案：使用 webkit 内核私有属性 -webkit-text-size-adjust:none; 再设置字体大小</p>
                </dd>
                <dt class="icon icon-arrow-r">页面内容的瞬间闪烁（FOUC）</dt>
                <dd>
                    <p>如果使用 import 方法对 CSS 进行导入，会导致某些页面在 Windows 下的 Internet Explorer 出现一些奇怪的现象:以无样式显示页面内容的瞬间闪烁，这种现象称之为文档样式短暂失效（Flash of Unstyled Content），简称为 FOUC</p>

                    <p>浏览器版本：IE</p>

                    <p>触发条件：
                        <br/>使用 import 方法导入样式表
                        <br/>将样式表放在页面底部
                        <br/>有几个样式表，放在 html 结构的不同位置
                    </p>

                    <p>解决方案：使用 link 标签将样式表放在文档 head 中</p>
                </dd>
            </dl>
        </section>
        <section class="document_section section">
        ...
        <section class="document_section section">
            ...
            <dl class="list">
                ...
                <dd>
                    ...
                    <p>空间还能够对文本和图像的过密产生一定的缓冲，让眼睛有休息的余地，将大大提高内容的可读性，
                        并给浏览者足够的思考内容的时间</p>
...
<!--                    <p>并非所有的图片都是可以平铺的，除非</p>-->
                ...
                <dd>
                    ...
                    <p>对某些非活动的图标或链接，同样可以通过降低对比度来实现“变灰（grayed out）”效果，让用户了解该选项当前不可用</p>
                ...
                <dd>
                    <ul>
                        ...
                        <li>了解页面中的关键元素</li>
                        <li>清楚地表达页面的内容，人类的注意力持续时间较短，通常来讲最多 9 秒钟——若是在这个时间段之后还是不能了解页面的主要内容，那么用户就会彻底放弃</li>
                    </ul>
                </dd>
                <dt class="icon icon-arrow-r">与布局相关的可用性</dt>
                <dd>
                    <ul>
                        <li>重要信息应放在显眼的位置，当然 Web 设计中“显眼”只是个相对概念，但通常，将用户最需要的信息尽可能地放在页面的最上部</li>
                ...
                <dd>
                    ...
                    <p>通常做法中，圆滑、缺乏粗细对比的 sans serif 字体被用作标题用字，而 serif 字体则常用作正文字体，这种设定在长段文章中的应用尤其广泛。不过在当今的 Web 时代，很多人的想法恰恰相反。</p>
                </dd>
                <dt class="icon icon-arrow-r">字体色深（type color）</dt>
                <dd>字体色深指的是文字的深浅程度。例如 Helvetica Light 这样的字体相对较浅，是由于字体较细因而含墨量较小。其它因素如字间距调整（kerning）、字间距（tracking）和行间距（leading）等也会影响人们感受到的字体的色深</dd>
                ...
                <dd>
                    ...
                    <ul>
                        <li>保持一行在 60 个字符以内。虽然在流式布局的网站设计中（尤其是 CSS 中问题百出的 min-width 和 max-width 属性）很难控制每行字数，但过长的一行文字不但会让读者难以理解，甚至会使其根本不愿阅读下去</li>
                        <li>避免每行字数过少，若将段落分布于 20 字的短行中，将导致行数激增，而过长的段落往往是人们无法忍受的，因而最好只在需要强调的文字中使用短行</li>
                        <li>英语书写的传统认为一个段落至少应由 3 到 4 句话组成。然而在 Web 中，过长的段落却不被看好。较短的段落在快捷的网络时代更适合迅速传达重要的讯息</li>
